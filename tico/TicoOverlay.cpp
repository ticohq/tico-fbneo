...
void TicoOverlay::RenderTitleCard(ImDrawList *dl, ImVec2 displaySize) {
    ...
    float AVAILABLE_TOP = 110.0f * scale;
    float cardWidth = displaySize.x * 0.75f; 
    
    // Robust word breaking and line filling
    ...
    for (size_t i = 0; i <= titleStr.length(); i++) {
        char c = (i < titleStr.length()) ? titleStr[i] : ' ';
        
        if (c == ' ' || c == '\n' || c == '\t' || i == titleStr.length()) {
            if (!currentWord.empty()) {
                std::string testLine = lines.back().empty() ? currentWord : (lines.back() + " " + currentWord);
                ImVec2 sz = ImGui::CalcTextSize(testLine.c_str());
                
                if (sz.x > cardWidth && !lines.back().empty()) {
                    if (lines.size() == 2) {
                        // We are already on line 2, and adding this word overflows it.
                        // We must truncate line 2 and break.
                        lines.back() += "...";
                        while (lines.back().length() > 4) {
                            ImVec2 tSz = ImGui::CalcTextSize(lines.back().c_str());
                            if (tSz.x <= cardWidth) break;
                            lines.back().erase(lines.back().length() - 4, 1);
                        }
                        currentWord.clear();
                        break;
                    } else {
    ...
    // If the loop finished but we still need to truncate at exactly 2 lines (e.g. huge single word overflow)
    // The loop above already handles the > cardWidth case organically.
    ...
    float lineSpacing = 4.0f * scale;
    float totalTextHeight = (lines.size() * fontSize) + ((lines.size() - 1) * lineSpacing);
    float cardY = (AVAILABLE_TOP - totalTextHeight) * 0.5f;
    ...
    float yCursor = currentY;
    for (const std::string &line : lines) {
        ImVec2 sz = ImGui::CalcTextSize(line.c_str());
        float textX = (displaySize.x - sz.x) * 0.5f;
        UIStyle::DrawTextWithShadow(dl, ImVec2(textX, yCursor), IM_COL32(200, 200, 200, 255), line.c_str());
...
void TicoOverlay::RenderHelpersBar(ImDrawList *dl, ImVec2 displaySize) {
    ...
    if (m_currentMenu == OverlayMenu::Settings) helpers.push_back({"DPAD", tr("emulator_change")});
...
#ifdef __SWITCH__
    std::string configPath = "sdmc:/tico/config/cores/fbneo.jsonc";
#else
